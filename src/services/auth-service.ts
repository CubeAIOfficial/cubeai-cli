...
export class AuthService extends Service {
    ...
    try {
      ...
      if (accessData) {
        ...
        for (const [userId, accesses] of Object.entries(accessMap)) {
          this.agentAccess.set(
            userId,
            (accesses as any[]).map((access) => ({
              ...access,
  ...
  getAgentPermissions(userId: string, agentId: string): Permission[] {
    ...
    if (
      !agentAccess ||
      (agentAccess.expiresAt && agentAccess.expiresAt < new Date())
    ) {
      return [];
    }
  ...
  async validateWebSocketConnection(
    ...
    try {
      ...
      const cachedUser = Array.from(this.userCache.values()).find(
        (user) => user.apiKey === token
      );
  ...
  getAuthStats(): {
    ...
    const agentAccessGrants = Array.from(this.agentAccess.values()).reduce(
      (total, accesses) => total + accesses.length,
      0
    );
